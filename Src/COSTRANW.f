...
C
      DO K=1,KC    
      DO L=2,LA
       DELCX(L,K)=CON1(L,K)-CON1(LWEST(L)   ,K)
...
C
     ...
     &            +COSMICXN(LEAST(L),K)*DELCX(LEAST(L),K) )
      ENDDO
      ENDDO
C
      DO K=1,KC    
...
C
      DO K=1,KC    
      DO L=2,LA
       DELCX(L,K)=CONCY(L,K)-CONCY(LWEST(L),K)
      ENDDO
      ENDDO
C
      IF(KC.GE.2)THEN
...
C
      DO K=1,KC    
      DO L=2,LA
       DELCX(L,K)=CONCZ(L,K)-CONCZ(LWEST(L)   ,K)
...
c
	...
	IF(UHDY2(L,K).GT.0.0)THEN
         CSTARD=CONCXYZ(L,K)
	   CSTARU=CONCXYZ(LWEST(L),K)
	   CSTARUU=CONCXYZ(LWEST(L),K)
	   IF(SUB(LWEST(L)).GT.0.5) CSTARUU=CONCXYZ(L-2,K)
         ...
         IF(SCB(L).GT.0.5)THEN
           ...
           ABSBOT=ABS(BOT)
           IF(SUB(LWEST(L)).GT.0.5.AND.ABSBOT.GE.1.E-23) THEN
             RVAL=TOP/BOT
             RMIN1=MIN(2.*RVAL,1.)
             RMIN2=MIN(RVAL,2.)
	...
	IF(UHDY2(L,K).LT.0.0)THEN
         CSTARD=CONCXYZ(LWEST(L),K)
	   CSTARU=CONCXYZ(L,K)
	   CSTARUU=CONCXYZ(L,K)
	   IF(SUB(LEAST(L)).GT.0.5) CSTARUU=CONCXYZ(LEAST(L),K)
         TMPVAL=1.+DELT*UHDY2(L,K)*DXYIU(L)*HUI(L)
         RLIM=0.
         IF(SCB(LWEST(L)).GT.0.5)THEN
           TOP=CSTARU-CSTARUU
           BOT=CSTARD-CSTARU
           IF(TOP.EQ.BOT) RLIM=1.
           ABSBOT=ABS(BOT)
           IF(SUB(LEAST(L)).GT.0.5.AND.ABSBOT.GE.1.E-23) THEN
             RVAL=TOP/BOT
             RMIN1=MIN(2.*RVAL,1.)
             RMIN2=MIN(RVAL,2.)
             RLIM=MAX(0.,RMIN1,RMIN2)
...
CX     &       +DELT*( (WFQC*FQC(L,K)+FUHU(L,K)-FUHU(LEAST(L),K)
...
C
      ...
      DO L=2,LA
CX      CH(L,K)=CONT(L,K)*H1P(L)
CX     &       +DELT*( (FUHU(L,K)-FUHU(LEAST(L),K)
      CH(L,K)=CON1(L,K)*H1P(L)
     &       +DELT*( ( RDZIC*FQC(L,K)+FUHU(L,K)-FUHU(LEAST(L),K)
     &               +FVHU(L,K)-FVHU(LNC(L),K))*DXYIP(L)
     &               +(FWU(L,K-1)-FWU(L,K))*RDZIC )

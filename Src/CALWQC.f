...
C     
        ...
        DO NW=1,NWQV
          IF(ISTRWQ(NW)==1)THEN
            ...
            DO ND=1,NDM  !K=2,KS, mid layers
              ...
              ENDDO
            ENDDO
            RCDZKMK=-DELT*CDZKMK(KC)  
            DO ND=1,NDM !K=KC, top layer
              LF=2+(ND-1)*LDM  
              LL=LF+LDM-1  
              DO L=LF,LL  
                CCLBTMP=RCDZKMK*HWQI(L)*AB(L,KC-1)  
                CCMBTMP=1.-CCLBTMP
                EEB=1./(CCMBTMP-CCLBTMP*CU1(L,KC-1))  
                WQV(L,KC,NW)=(WQV(L,KC,NW)-CCLBTMP*WQV(L,KC-1, 1))*EEB
              ENDDO
            ENDDO
          ENDIF
        ENDDO
      ...
      !IF(ISWQLVL.EQ.1)THEN  
        ...
        !    WQV(L,K,20)=(WQV(L,K,20)-CCLBTMP*WQV(L,K-1,20))*EEB  
        !    WQV(L,K,21)=(WQV(L,K,21)-CCLBTMP*WQV(L,K-1,21))*EEB  
        !    WQV(L,K,21)=(WQV(L,K,22)-CCLBTMP*WQV(L,K-1,22))*EEB  
        !  ENDDO  
        !ENDDO  
        !DO ND=1,NDM  
        !  LF=2+(ND-1)*LDM  

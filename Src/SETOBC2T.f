...
C
      ...
      ENDDO
      CET=0.5*D_LTD2*G*HRUO(LEAST(L))*RCX(LEAST(L))*HUTMP(LEAST(L))
      IF(ISPBW(LL).GE.1)THEN
       TMP=D_LTD2*SQRT(G*HMU(LEAST(L)))*DXIU(LEAST(L))
       CC(L)=CET*(1.+TMP)/TMP
       CE(L)=-CET
       FP1(L)=CET*(2.*FP1(L)
     & -SQRT(G*HMU(LEAST(L)))*FUHDYE(LEAST(L))*DYIU(LEAST(L))*HUI(LEAST(L)))/TMP
      ELSE
       FP1(LEAST(L))=CET*FP1(L)
       FP1(L)=CC(L)*FP1(L)
      ENDIF
      ENDDO
C

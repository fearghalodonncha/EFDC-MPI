  ...
  300 CONTINUE  
      ...
      DO IS=1,ISECVPV  
        ...
        DO NN=1,NIJVPV(IS)  
          ...
          LS=LSC(L)  
          DO K=1,KC  
            RVELN(K,NN)=50.*((UHLPF(LEAST(L),K)+UHLPF(L,K))*COSC  
     &          +(VHLPF(LN,K)+VHLPF(L,K))*SINC)/HLPF(L)  
            RVELT(K,NN)=-50.*((UHLPF(LEAST(L),K)+UHLPF(L,K))*SINC  
     &          -(VHLPF(LN,K)+VHLPF(L,K))*COSC)/HLPF(L)  
            RW(K,NN)=50.*(WLPF(L,K)+WLPF(L,K-1))  
            PVELN(K,NN)=50.*((UVPT(LEAST(L),K)+UVPT(L,K))*COSC  
     &          +(VVPT(LN,K)+VVPT(L,K))*SINC)/HLPF(L)  
            PVELT(K,NN)=-50.*((UVPT(LEAST(L),K)+UVPT(L,K))*SINC  

...
C  
     ...
     &          +MIN(VHVW,0.)*QQ1(L,K)  
            FUHV(L,K)=MAX(UHUW,0.)*QQL1(LWEST(L),K)  
     &          +MIN(UHUW,0.)*QQL1(L,K)  
        ...
        IF(ISCDCA(0).EQ.1)THEN  
          DO K=1,KS  
            DO L=2,LA  
              ...
              FUHV(L,K)=UHUW*(QQL(LWEST(L),K)+QQL(L,K))  
     ...
     &            +(FWQQL(L,K)-FWQQL(L,K+1))*DZIG(K))*DXYIP(L)  
          ...
          DO K=1,KS  
            DO L=2,LA  
              ...
              PQQU=AV(L,K)*DZIGSD4(K)*(U(LEAST(L),K+1)-U(LEAST(L),K)+U(L,K+1)-
...
C  
     ...
     &            +AV(L,K)*DZIGSD4(K)*(U(LEAST(L),K+1)-U(LEAST(L),K)+U(L,K+1)-
     &      U(L,K))**2+AV(L,K)*DZIGSD4(K)*(V(LN,K+1)-V(LN,K)+V(L,K+1)

...
C  
      DO L=2,LA  
        LN=LNC(L)  
        FP(L)=DELTI*DXYP(L)*P1(L)-0.5*G*(UHDY1E(LEAST(L))-UHDY1E(L)  
     &                                  +VHDX1E(LN )-VHDX1E(L))  
      ENDDO  
C  
...
C  
      ...
      DO L=2,LA  
        ...
        FP(L)=FP(L)-C1*(FUHDYE(LEAST(L))-FUHDYE(L)  
     &                 +FVHDXE(LN )-FVHDXE(L)  
     &                -2.0*QSUME(L) )  
      ENDDO  
C
...
C  
      IF(IRVEC.EQ.99)THEN  
        DO L=2,LA  
          ...
          CCW(L)=CW(L)/SQRT( CC(L)*CC(LWEST(L)   ) )  
          CCE(L)=CE(L)/SQRT( CC(L)*CC(LEAST(L)   ) )  
      ...
      IF(ISNEGH.GE.1)THEN  
        ...
        DO L=2,LA  
          IF(HP(L).LT.0.)THEN  
            ...
            WRITE (6,6062)IL(L),JL(L),HU(LEAST(L)),H1U(LEAST(L))  
            ...
            WRITE (8,6062)IL(L),JL(L),HU(LEAST(L)),H1U(LEAST(L))  

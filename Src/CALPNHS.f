...
C  
     ...
     &      +0.5*U(LEAST(L),KC)*(P(LEAST(L))-P(L))*DXIU(LEAST(L))  
     &      +0.5*U(L,KC)*(P(L)-P(LWEST(L)))*DXIU(L)  
     ...
     &          +0.5*U(LEAST(L),K)*(P(LEAST(L))-P(L))*DXIU(LEAST(L))  
     &          +0.5*U(L,K)*(P(L)-P(LWEST(L)))*DXIU(L)  
     &          +0.5*V(LN,K)*(P(LN)-P(L))*DYIV(LN)  
     &          +0.5*V(L,K)*(P(L)-P(LS))*DYIV(L) )  
     &          +(1.-ZZ(K))*( DELTI*(BELV(L)-BELV1(L))  
     &          +0.5*U(LEAST(L),K)*(BELV(LEAST(L))-BELV(L))*DXIU(LEAST(L))  
     &          +0.5*U(L,K)*(BELV(L)-BELV(LWEST(L)))*DXIU(L)  

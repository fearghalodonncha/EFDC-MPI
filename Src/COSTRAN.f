...
C  
      DO K=1,KC  
        DO L=2,LA  
          FUHU(L,K)=MAX(UHDY2(L,K),0.)*CONCXYZ(LWEST(L),K)  
...
C  
      DO K=1,KC  
        DO L=2,LA  
          ...
          AUHU=ABS(UHDY2(L,K))  
          AVHV=ABS(VHDX2(L,K))  
        UTERM=AUHU*(1.-DELTA*AUHU*DXYIU(L)*HUI(L))*(CON(L,K)-CON(LWEST(L),K))  
     ...
     &          -QSUMPAD(LS ,K)*CON(LS ,K))  
          ...
          IF(ISFCT(MVAR).GE.2)THEN  
            ...
            ENDIF  
          ELSE  
            UHU=UTERM/(CON(L,K)+CON(LWEST(L),K)+BSMALL)  
            VHV=VTERM/(CON(L,K)+CON(LS,K)+BSMALL)  
            FUHU(L,K)=MAX(UHU,0.)*CON(LWEST(L),K)  
     &          +MIN(UHU,0.)*CON(L,K)  

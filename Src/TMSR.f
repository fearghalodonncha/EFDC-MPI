...
C  
     ...
     &    TFNTXBT,TFNTXBF,TFNTXBC,TFNTXBP  
      ...
      CHARACTER*2,SAVE,ALLOCATABLE,DIMENSION(:)::CNTMSR_GL  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::PORH  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::QCHANUIJ  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::QCHANVIJ  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::SEDSND  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::SEDSNDB  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::TXBC  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::TXBF  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::TXBP  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::TXBT  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::TXWC  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::TXWF  
      REAL,SAVE,ALLOCATABLE,DIMENSION(:)::TXWP  
      CHARACTER*12,SAVE,ALLOCATABLE,DIMENSION(:)::FNW3D
      IF(.NOT.ALLOCATED(CNTMSR_GL))THEN
        ALLOCATE(CNTMSR_GL(MLTMSRM))
        ALLOCATE(PORH(KBM))  
...
C
      DO MLTM=1,MLTMSR_GL
        ...
        CCHTMF=CZTT(MFDIG)
        CCHTMS=CZTT(MSDIG)
        CNTMSR_GL(MLTM)= CCHTMF // CCHTMS
      ENDDO
      DO MLTM = 1,MLTMSR
         II = MLTM_GL(MLTM)
         CNTMSR(MLTM) = CNTMSR_GL(II)
      END DO
C
...
C          ENDIF
     ...
     &                           CNTMSR(MLTM) // '.OUT'
        ...
        IF(MTMSRU(MLTM).EQ.1)THEN
          ...
          FNW3D(MLTM)='W3DTS' // CNTMSR(MLTM) // '.OUT'
        ENDIF
      ...
      DO MLTM=1,MLTMSR
        IF(MTMSRC(MLTM).EQ.1)THEN
          IF(ISTRAN(1).GE.1)THEN
            ...
            WRITE (11,103)XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
            WRITE (11,102) CTUNIT
            CLOSE(11)
          ENDIF
          IF(ISTRAN(2).GE.1)THEN
            ...
            WRITE (21,103)XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
            WRITE (21,102) CTUNIT
            CLOSE(21)
          ENDIF
          IF(ISTRAN(3).GE.1)THEN
            ...
            WRITE (31,103)XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
            WRITE (31,102) CTUNIT
            CLOSE(31)
          ENDIF
          IF(ISTRAN(4).GE.1)THEN
          ...
          IF(ISTRAN(6).GE.1)THEN
            ...
            WRITE (41,103)XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
            WRITE (41,102) CTUNIT
            CLOSE(41)
          ENDIF
          IF(ISTRAN(7).GE.1)THEN
...
C            CLOSE(41)
          ...
          IF(ISTRAN(8).GE.1)THEN
            ...
            WRITE (42,103) XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
            WRITE (42,102) CTUNIT
            CLOSE(42)
            FNNHX(MLTM)='NHXTS' // CNTMSR(MLTM) // '.OUT'
            ...
            WRITE (43,103) XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
            WRITE (43,102) CTUNIT
            CLOSE(43)
          ENDIF
          IF(ISTRAN(5).GE.1)THEN
     ...
     &                           CNTMSR(MLTM) // '.OUT'
              ...
              WRITE (51,103) XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
              WRITE (51,102) CTUNIT
              CLOSE(51)
              FNTXWT(MLTM,NT)='TXWT' // CNTOX(NT) // 'TS' // 
     &                           CNTMSR(MLTM) // '.OUT'
     ...
     &                           CNTMSR(MLTM) // '.OUT'
        ...
        IF(MTMSRA(MLTM).EQ.1)THEN
          ...
          WRITE (61,103) XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
          WRITE (61,102) CTUNIT
          CLOSE(61)
          FNAVB(MLTM)='AVBTS' // CNTMSR(MLTM) // '.OUT'
          ...
          WRITE (71,103) XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
          WRITE (71,102) CTUNIT
          CLOSE(71)
        ENDIF
        IF(MTMSRP(MLTM).EQ.1)THEN
        ...
        IF(MTMSRU(MLTM).EQ.1)THEN
          ...
          CLOSE(51)
          FNW3D(MLTM)='W3DTS' // CNTMSR(MLTM) // '.OUT'
          OPEN(51,FILE=FNW3D(MLTM),STATUS='UNKNOWN')
          CLOSE(51,STATUS='DELETE')
          OPEN(51,FILE=FNW3D(MLTM),STATUS='UNKNOWN')
          WRITE (51,100) ' INT MODE VERTICAL VEL (CM/S) TS, K=1,KC-1'
          WRITE (51,101) CLTMSR(MLTM)
          WRITE (51,103) XPAR(ILTMSR(MLTM)),YPAR(JLTMSR(MLTM))
          WRITE (51,102) CTUNIT
          CLOSE(51)
...
C          GWELTMP=AGWELV(L)-BELAGW(L)
     ...
     &          TMPVAL,VDWASTE(L)
        ...
        IF(MTMSRUE(MLTM).EQ.1)THEN
          ...
          UTMP1=50.*(UHDYE(LEAST(L))+UHDYE(L))/(DYP(L)*HP(L))
...
c          UTMP=0.5*STCUV(L)*(U(LEAST(L),1)+U(L,1))+1.E-12
...
c          IF(VHDXE(L).NE.0.0)CQBEDLOADY=QBEDLOADY/VHDXE(L)
     ...
     &                 CQBEDLOADXT,CQBEDLOADYT
        ...
        IF(MTMSRU(MLTM).EQ.1)THEN
          ...
          OPEN(511,FILE=FNW3D(MLTM),POSITION='APPEND')
          RUVTMP=50.
          IF(SPB(L).EQ.0) RUVTMP=100.
          DO K=1,KC
           UTMP1=RUVTMP*(U(LEAST(L),K)+U(L,K))
          ...
          WRITE(511,201)TIME,(W(L,K)*100,K=1,KS)
          CLOSE(41)
          CLOSE(51)
          CLOSE(511)
        ENDIF
...
C 
c      DEALLOCATE(PORH)  
c      DEALLOCATE(QCHANUIJ)  
c      DEALLOCATE(QCHANVIJ)  
c      DEALLOCATE(SEDSND)  
c      DEALLOCATE(SEDSNDB)  
c      DEALLOCATE(TXBC)  
c      DEALLOCATE(TXBF)  
c      DEALLOCATE(TXBP)  
c      DEALLOCATE(TXBT)  

...
!     New variable needed to find the the depth to the thermocline
      ...
      REAL::Knot,Knottherm,Knotmet,BVFZ,KnotZ,Kv0
...
C   SMMAX    =      4.9639  
C  
      OPEN(753,FILE='CALAVB.OUT',POSITION='APPEND')
      QQIMAX=1./QQMIN  
      AVMAX=AVO  
      ABMAX=ABO  
      AVMIN=10.  
...
! Move specification of these calibration parameters to VINLETPAR.INP to allow
! ensembles
...
!Calculation of K(Ztherm): KZTH
     ...
     &EXP(2.*ZTHERM(L)/CP1/SURFVEL) !Ri(Ztherm)
               ...
               IF(BVF>0.0)THEN
                 SFAB=alpha_vlet*KZTH*(BVFZtherm/BVF)**P2_MET !K(z) for Ztherm > Z >= Zmet
               ELSE
...
!     &    1.0E-6) !Brunt Vaisala frequency at Zmet: BFV(Zmet)
     ...
     &KZM*(BVFZmet/BVF)**P2_HYPO !K(z) for Zmet > Z >= Zmax (or HP(L)) 
               ELSE
                 SFAB=0.0
               ENDIF
             ENDIF
...
!Debugging vertical profiles
              if(MOD(n-1,86400)==0.AND.(L==1963.OR.L==130))then
               write(753,'(4(i3,1x),12(1x,e9.3))')N/86400,IL(L),JL(L),K,
     &            RIQprint,BVF,DZIG(K)*(B(L,K+1)-B(L,K)),B(L,K),TEM(L,K)
     &            ,SFAB,SFAB,BVFZtherm,BVFZmet,Knot,Knottherm,Knotmet
              endif
      ...
      ENDIF
!      IF(MOD(n-1,1000)==0)
!     &write(6,'("AVmax,ABmax: ",2(e9.3,1x))')AVMAX,ABMAX
! *** NOW APPLY MAXIMUM, IF REQUIRED
      ...
      ENDIF
!      IF(MOD(N-1,10000)==0)
!     &write(6,'("AVmax,ABmax: ",e9.3,2(1x,i4),1x,e9.3,2(1x,i4))')
!     &MAXVAL(AV(2:LA,1:KC)),MAXLOC(AV(2:LA,1:KC)),
!     &MAXVAL(AB(2:LA,1:KC)),MAXLOC(AB(2:LA,1:KC))

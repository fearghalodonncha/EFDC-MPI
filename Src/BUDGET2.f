...
C  
     ...
     &          -MAX(VHDX2(LN,K),0.)*SND1(L,K,NT)*DZC(K)  
      ...
      DO K=1,KC  
        DO LL=1,NCBW  
          L=LCBW(LL)  
          VOLMOUT=VOLMOUT-UHDY2(LEAST(L),K)*DZC(K)  
          SMASSOUT=SMASSOUT-MIN(UHDY2(LEAST(L),K),0.)*SAL1(LEAST(L),K)*DZC(K)  
     &        -MAX(UHDY2(LEAST(L),K),0.)*SAL1(L,K)*DZC(K)  
C  
C  ADDED SEDIMENT FLUXES         DLK 10/15  
C  
          DO NT=1,NSED  
            SEDOUT=SEDOUT-MIN(UHDY2(LEAST(L),K),0.)*SED1(LEAST(L),K,NT)*DZC(K)  
     &          -MAX(UHDY2(LEAST(L),K),0.)*SED1(L,K,NT)*DZC(K)  
          ENDDO  
          DO NT=1,NSND  
            SEDOUT=SEDOUT-MIN(UHDY2(LEAST(L),K),0.)*SND1(LEAST(L),K,NT)*DZC(K)  
     &          -MAX(UHDY2(LEAST(L),K),0.)*SND1(L,K,NT)*DZC(K)  
          ENDDO  
        ENDDO  
      ENDDO  
      DO K=1,KC  
...
C  
     ...
     &          +MAX(UHDY2(L,K),0.)*SED1(LWEST(L),K,NT)*DZC(K)  
          ENDDO  
          DO NT=1,NSND  
            SEDOUT=SEDOUT+MIN(UHDY2(L,K),0.)*SND1(L,K,NT)*DZC(K)  
     &          +MAX(UHDY2(L,K),0.)*SND1(LWEST(L),K,NT)*DZC(K)  
          ENDDO  
        ENDDO  
      ENDDO  
      DO K=1,KC  

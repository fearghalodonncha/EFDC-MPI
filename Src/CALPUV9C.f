...
C
     ...
     &        -SBX(L)*HU(L)*GP*((BI2(L)+BI2(LWEST(L)))*(HP(L)-HP(LWEST(L)))  
     &        +2.0*HU(L)*(BI1(L)-BI1(LWEST(L)))    
     &        +(BE(L)+BE(LWEST(L)))*(BELV(L)-BELV(LWEST(L)))))  
        ENDDO  
        DO L=2,LA  
          LS=LSC(L)  
...
C  
      ...
      DO L=2,LA  
        ...
        FUHDYE(L)=UHDY1E(L)  
     &      -DELTD2*SUB(L)*HRUO(L)*HUTMP(L)*(P1(L)-P1(LWEST(L)))  
...
C
     ...
     &        P1(LWEST(L)),TSX1(L),TBX1(L),FCAXE(L),FPGXE(L),FXE(L)  
        ENDDO  
        CLOSE(1)  
        IF(N.EQ.1)THEN  
...
C  
      DO L=2,LA  
        ...
        SBX(L)=SBXO(L)  
        SBY(L)=SBYO(L)  
        SUB(LEAST(L))=SUBO(LEAST(L))  
        SVB(LN)=SVBO(LN)  
        SBX(LEAST(L))=SBXO(LEAST(L))  
        SBY(LN)=SBYO(LN)  
      ENDDO  
C
...
C  
      DO L=2,LA  
        LN=LNC(L)  
        FP1(L)=DELTI*DXYP(L)*P1(L)-0.5*G*(UHDY1E(LEAST(L))-UHDY1E(L)  
     &                                   +VHDX1E(LN )-VHDX1E(L))  
      ENDDO  
C  
...
C  
 1000 CONTINUE  
      ...
      DO L=2,LA  
        LN=LNC(L)  
        FP(L)=FP1(L)-C1*(FUHDYE(LEAST(L))-FUHDYE(L)  
     &                  +FVHDXE(LN )-FVHDXE(L)  
     &                -2.0*QSUME(L) )  
      ENDDO  
C
...
C
      IF(ISDSOLV.GE.1.AND.DEBUG)THEN  
        ...
        WRITE(1,1001)N,ISTL_  
        DO L=2,LA  
          WRITE(1,1001)IL(L),JL(L),FP1(L),FUHDYE(L),FUHDYE(LEAST(L)),  
...
C
      ...
      DO L=2,LA  
        ...
        CE(L)=C1*SUB(LEAST(L))*HRUO(LEAST(L))*RCX(LEAST(L))*HUTMP(LEAST(L))  
      ENDDO  
      DO L=2,LA  
        LN=LNC(L)  
...
C  
     ...
     &      -DELTD2*HRUO(L)*RCX(L)*HUTMP(L)*(P(L)-P(LWEST(L))) )  
...
C  
     ...
     &        -0.5*(UHDYE(LEAST(L))+UHDY2E(LEAST(L))-UHDYE(L)-UHDY2E(L)  
     ...
     &        -0.5*(UHDYE(LEAST(L))+UHDY1E(LEAST(L))-UHDYE(L)-UHDY1E(L)  
...
C  
      ...
      DO L=2,LA  
        LS=LSC(L)  
        HU(L)=0.5*(DXYP(L)*HP(L)+DXYP(LWEST(L))*HP(LWEST(L)))*DXYIU(L)  
      ...
      IF(ISNEGH.GE.1)THEN  
        ...
        DO L=2,LA  
          IF(HP(L).LT.0.)THEN  
            ...
            WRITE (6,6061)IL(L),JL(L),ISCDRY(LWEST(L)),HU(L),H1U(L)  
            WRITE (6,6062)IL(L),JL(L),ISCDRY(LEAST(L)),HU(LEAST(L)),H1U(LEAST(L))  
            ...
            WRITE (8,6061)IL(L),JL(L),ISCDRY(LWEST(L)),HU(L),H1U(L)  
            WRITE (8,6062)IL(L),JL(L),ISCDRY(LEAST(L)),HU(LEAST(L)),H1U(LEAST(L))  

...
C  
          VOIDCON1=BEDPORC/(1.0-BEDPORC)  
          DO L=2,LA  
            K=KBT(L)  
            HBEDTMP=(1.0+VOIDCON1)*HBED(L,K)/(1.0+VDRBED(L,K))  
            TMPVALO=VDRBED(L,K)*HBED(L,K)/(1.0+VDRBED(L,K))  
            TMPVALN=VOIDCON1*HBEDTMP/(1.0+VOIDCON1)  
...
C  
        IF(IBMECH.EQ.1.AND.IWRSP(1).LT.98)THEN  
C  
            ...
            DO K=1,KB  
              DO L=2,LA  
                IF(K.LE.KBT(L))THEN  
                  ...
                  TMPTOP=1.0+VDRBED(L,K)  
                  TMPBOT=1.0+VDRBED1(L,K)  
...
C  
          DO K=1,KB  
            DO L=2,LA  
              IF(K.LE.KBT(L))THEN  
                DZBTR(L,K)=HBED(L,K)/(1.0+VDRBED(L,K))
                DZBTR1(L,K)=HBED1(L,K)/(1.0+VDRBED1(L,K))
              ENDIF  
            ENDDO  
          ENDDO  
C  
...
C  
          DO K=1,KB  
            DO L=2,LA  
              IF(K.LT.KBT(L))THEN  
                ACOEF(L,K)=2.0*COEFSK(L,K)/(DZBTR(L,K)+DZBTR(L,K+1))  
     ...
     &              (DZBTR(L,K)+DZBTR(L,K+1))  
          ...
          DO L=2,LA  
            K=KBT(L)  
            ACOEF(L,K)=2.0*COEFSK(L,K)/DZBTR(L,K)  
...
C  
          DO K=1,KB  
            DO L=2,LA  
              IF(K.LE.KBT(L))THEN  
                HBED(L,K)=HBED1(L,K)*(1.+VDRBED(L,K))/(1.0+VDRBED1(L,K))
              ELSE  
                HBED(L,K)=0.0  
              ENDIF  
            ENDDO  
...
C  
     ...
     &              FHYDCN(VDRBEDSED(L,K),BMECH4,BMECH5,BMECH6,IBMECHK)
                TMPVAL=VDRBEDSND(L,K)/(1.0+VDRBEDSND(L,K))
...
C  
          DO K=1,KB  
            DO L=2,LA  
              IF(K.LE.KBT(L))THEN  
                HBED(L,K)=HBED1(L,K)*(1.0+VDRBED(L,K))/
     .                               (1.0+VDRBED1(L,K))
               ELSE  
                HBED(L,K)=0.0
              ENDIF  

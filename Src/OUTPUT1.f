...
C  
      DO KK=1,KC,KS  
        DO L=2,LA  
          PAM(L)=0.5*(UHLPF(L,KK)+UHLPF(LEAST(L),KK))/HMP(L)  
        ENDDO  
        WRITE (7,58) KK  
        WRITE (7,757)N  
        CALL PPLOT (2)  
...
C  
      DO KK=1,KC,KS  
        DO L=2,LA  
          PAM(L)=0.5*(UVPT(L,KK)+UVPT(LEAST(L),KK))/HMP(L)  
        ENDDO  
        WRITE (7,1458) KK  
        WRITE (7,757)N  
        CALL PPLOT (2)  
...
C  
      DO KK=1,KC,KS  
        DO L=2,LA  
          PAM(L)=0.5*(UHLPF(L,KK)+UHLPF(LEAST(L),KK)+UVPT(L,KK)  
     &        +UVPT(LEAST(L),KK))/HMP(L)  
        ENDDO  
        WRITE (7,1558) KK  
        WRITE (7,757)N  
        CALL PPLOT (2)  

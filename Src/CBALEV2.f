...
C  
     ...
     &        +0.5*HP(L)*(Z(K)+Z(K-1)) )*B1(L,K)  
      ...
      DO K=1,KC  
        DO LL=1,NCBW  
          L=LCBW(LL)  
          VOLOUTE=VOLOUTE-UHDY2(LEAST(L),K)*DZC(K)  
          SALOUTE=SALOUTE-MIN(UHDY2(LEAST(L),K),0.)*SAL1(LEAST(L),K)*DZC(K)  
     &        -MAX(UHDY2(LEAST(L),K),0.)*SAL1(L,K)*DZC(K)  
          DYEOUTE=DYEOUTE-MIN(UHDY2(LEAST(L),K),0.)*DYE1(LEAST(L),K)*DZC(K)  
     &        -MAX(UHDY2(LEAST(L),K),0.)*DYE1(L,K)*DZC(K)  
          PPEOUTE=PPEOUTE-UHDY2(LEAST(L),K)*G*DZC(K)*(0.5*(BELV(L)+BELV(LEAST(L)))  
     &        +0.125*(HP(L)+H2P(L)+HP(LEAST(L))+H2P(LEAST(L)))*(Z(K)+Z(K-1)) )  
          BBEOUTE=BBEOUTE-MIN(UHDY2(LEAST(L),K),0.)*DZC(K)*GP*( BELV(LEAST(L))  
     ...
     &        +MAX(UHDY2(L,K),0.)*DYE1(LWEST(L),K)*DZC(K)  
          PPEOUTE=PPEOUTE+UHDY2(L,K)*G*DZC(K)*( 0.5*(BELV(L)+BELV(LWEST(L)))  

...
C  
C   04/19/2017  FEARGHAL O'DONNCHA (feardonn@ie.ibm.com)
C  EXTENDED CAPABIILITIES TO RESTART TO MPI PARALLEL EFDC
C  ASSUMES THAT CONFIGURATION OF EFDC SUB-DOMAINS DOES NOT CHANGE
C  BETWEEN RESTART. I.e AT END OF SIMULATION EACH SUBDOMAIN WRITES IT'S
C  OWN CHUNK OF THE SOLUTION TO FILE TO BE READ IN THE SAME MANNER AT
C  THE BEGINNING OF THE NEXT SOLUTION
      USE GLOBAL  
      ...
      TDUMMY=0.
C  
      IF (PARTID == 0) PRINT *,'READING RESTART FILE: RESTART.rst'
      OPEN(1,FILE='RESTART'//ANS(PARTID2)//'.rst',STATUS='UNKNOWN')  
      ISBELVC=0  
      ...
      DO L=2,LA  
        ...
        ENDIF  
        IF(ISCI(2).EQ.1)THEN  
          READ(1,*,ERR=1015)(TEM(L,K),K=1,KC),TEMB(L)  
...
C  
 ...
 6667 FORMAT(7I5,2X,E12.4,2X,E12.4)  
      ...
      DO L=2,LA  
        LS=LSC(L)  
        H1U(L)=0.5*(DXP(L)*DYP(L)*H1P(L)+DXP(LWEST(L))*DYP(LWEST(L))*H1P(LWEST(L)))  
     &      /(DXU(L)*DYU(L))  
        H1V(L)=0.5*(DXP(L)*DYP(L)*H1P(L)+DXP(LS )*DYP(LWEST(L))*H1P(LS ))  
     &      /(DXV(L)*DYV(L))  
        P1(L)=G*(H1P(L)+BELV(L))  
        HU(L)=0.5*(DXP(L)*DYP(L)*HP(L)+DXP(LWEST(L))*DYP(LWEST(L))*HP(LWEST(L)))  
     &      /(DXU(L)*DYU(L))  
        HV(L)=0.5*(DXP(L)*DYP(L)*HP(L)+DXP(LS )*DYP(LWEST(L))*HP(LS ))  
     &      /(DXV(L)*DYV(L))  
        P(L)=G*(HP(L)+BELV(L))  
        HPI(L)=1./HP(L)  
        HUI(L)=1./HU(L)  
...
C  
      ...
      DO K=1,KS  
        ...
        DO L=2,LA  
          LN=LNC(L)  
          W(L,K)=SWB(L)*(W(L,K-1)  
     &        -RDZC*(UHDY(LEAST(L),K)-UHDY(L,K)-UHDYE(LEAST(L))+UHDYE(L)  
     &        +VHDX(LN,K)-VHDX(L,K)-VHDXE(LN)+VHDXE(L))*DXYIP(L))  
     &        +SWB(L)*( QSUM(L,K)-RDZC*QSUME(L) )*DXYIP(L)  
          W1(L,K)=SWB(L)*(W1(L,K-1)  
     &        -RDZC*(UHDY1(LEAST(L),K)-UHDY1(L,K)-UHDY1E(LEAST(L))+UHDY1E(L)  
          ...
          IF(HP(L).LE.HDRY)THEN  
            ...
            IF(DHPDT.GT.0.0)THEN  
              ...
              IF(SUBO(L).GT.0.5)THEN  
                IF(UHDYE(L).GT.0.0.AND.HP(LWEST(L)).GT.HDRY2)THEN  
                  SUB(L)=1.  
                  SBX(L)=1.  
                ENDIF  
              ENDIF  
              IF(SUBO(LEAST(L)).GT.0.5)THEN  
                IF(UHDYE(LEAST(L)).LT.0.0.AND.HP(LEAST(L)).GT.HDRY2)THEN  
                  SUB(LEAST(L))=1.  
                  SBX(LEAST(L))=1.  
                ENDIF  
              ENDIF  
              ...
              IF(SVBO(LN).GT.0.5)THEN  
                ...
                ENDIF  
              ENDIF  
              RDRY=SUB(L)+SUB(LEAST(L))+SVB(L)+SVB(LN)  
              IF(RDRY.LT.0.5)THEN  
                ISCDRY(L)=1  
              ELSE  
                ISCDRY(L)=0  

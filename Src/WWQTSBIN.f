...
C      DO M=1,IWQTS
      DO NW=1,NWQVM
        IF(ISTRWQ(NW)/=0)WQVSUM(:,:,NW)=WQVSUM(:,:,NW)+WQVO(:,:,NW)
      ENDDO
      DO LL=2,LA
        DO K=1,KC
C          LL=LWQTS(M)
...
C OPEN WQ ASCII FILE:
        OPEN(1234,FILE='WQWCTS1'//ans(partid2)//'.OUT',STATUS='UNKNOWN',POSITION='APPEND')
C
...
C WRITE TO ASCII FILE WQWCTS.OUT HERE:
            IF(ISASCII)THEN
             WRITE(1,71) XPAR(IL(LL)),YPAR(JL(LL)),K,TIMTMP, CHLWQ,TOCWQSUM(LL,K),
...
C     +       TPWQSUM(LL,K),WQVSUM(LL,K,9),POPSUM(LL,K),WQVSUM(LL,K,10),
     ...
     +       WQTEMSUM(LL,K)
           ...
           ELSE
            IF(ISASCII)THEN
             WRITE(1234,71) XPAR(IL(LL)),YPAR(JL(LL)),K,TIMTMP, CHLWQ,TOCWQSUM(LL,K),
     +       WQVSUM(LL,K,6), TPWQSUM(LL,K), WQVSUM(LL,K,9),
     +       WQVSUM(LL,K,10), PO4DWQSUM(LL,K), CHLC, TNWQSUM(LL,K),
     +       (WQVSUM(LL,K,NW),NW=13,15), TSIWQSUM(LL,K),
     +       (WQVSUM(LL,K,NW),NW=16,17), SADWQSUM(LL,K), BOD5SUM(LL,K),

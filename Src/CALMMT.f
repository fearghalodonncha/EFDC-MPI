  ...
  100 CONTINUE  
      IF(NTSMMT.LT.NTSPTC)THEN  
        DO L=2,LA  
          ...
          UTMP1=0.5*(UHDYE(LEAST(L))+UHDYE(L))/(DYP(L)*HP(L))  
        ...
        DO K=1,KC  
          DO L=2,LA  
            LS=LSC(L)  
            AHULPF(L,K)=AHULPF(L,K)+0.5*(AH(L,K)+AH(LWEST(L),K))  
     ...
     &          WIRT(LS,K))  
            VPY(L,K)=VPY(L,K)+0.25*(W(L,K)+W(LWEST(L),K))*(UIRT(L,K+1)+
     &          UIRT(L,K))  
          ENDDO  
        ENDDO  
        DO K=1,KC  
...
C  
     ...
     &          -0.25*(VTLPF(L,K+1)+VTLPF(L,K))*(WLPF(L,K)+WLPF(LS,K))  
            VPY(L,K)=VPY(L,K)  
     &          -0.25*(WTLPF(L,K)+WTLPF(LWEST(L),K))*(ULPF(L,K+1)+ULPF(L,K))
          ENDDO  
        ENDDO  
        DO K=1,KC  
          DO L=2,LA  
            ...
            LSW=LSWC(L)  
            VPZ(L,K)=VPZ(L,K)  
     &          -0.25*(UTLPF(L,K)+UTLPF(LS,K))*(VLPF(L,K)+VLPF(LWEST(L),K))  
            ! *** DSLLC BEGIN BLOCK
            TMPVAL=1.+SUB(L)+SVB(L)+SUB(L)*SVB(L)
            HPLW =SUB(L)*HP(LWEST(L))  
            HPLS =SVB(L)*HP(LS)
            HPLSW=SUB(L)*SVBO(L)*HP(LSW)
            HMC=(HP(L)+HPLW+HPLS+HPLSW)/TMPVAL
            VPZ(L,K)=VPZ(L,K)*HMC*SUB(L)*SUB(LS)*SVB(L)*SVB(LWEST(L))  
            ! *** DSLLC END BLOCK
          ENDDO  
        ENDDO  
        DO K=1,KC  
     ...
     &          -(VPZ(LEAST(L),K)-VPZ(L,K))/DXV(L)  
        ...
        DO K=1,KS  
          DO L=2,LA  
            LS=LSC(L)  
            LN=LNC(L)  
            WVPT(L,K)=(VPY(LEAST(L),K)-VPY(L,K))/DXP(L)-(VPX(LN,K)-VPX(L,K)
     &          )/DYP(L)  
          ENDDO  
        ENDDO  
      ENDIF  
      ...
      DO K=1,KC  
        DO LL=1,NPBW  
          L=LPBW(LL)  
          QXW=QXW+UHLPF(LEAST(L),K)*DZC(K)*DYU(LEAST(L))  
          QXWVP=QXWVP+UVPT(LEAST(L),K)*DZC(K)*DYU(LEAST(L))  
        ENDDO  
      ENDDO  
      QXE=0.  
      QXEVP=0.  

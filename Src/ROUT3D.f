...
C  
      ...
      IF(IS3DUUU.GE.1)THEN  
        ...
        DO K=1,KC  
          DO L=2,LA  
            TMPVAL=0.5*(ULPF(L,K)+ULPF(LEAST(L),K))  
     ...
     &          +0.5*(ULPF(LEAST(L),K)*(HLPF(LEAST(L))+BELV(LEAST(L))  
     &          -HLPF(L)-BELV(L))*DXIU(LEAST(L))  
     &          +ULPF(L,K)*(HLPF(L)+BELV(L)  
     &          -HLPF(LWEST(L))-BELV(LWEST(L)))*DXIU(L)  
     &          +VLPF(LN,K)*(HLPF(LN)+BELV(LN)  
     &          -HLPF(L)-BELV(L))*DYIV(LNC(L))  
     &          +VLPF(L,K)*(HLPF(L)+BELV(L)  
     &          -HLPF(LS)-BELV(LS))*DYIV(L)) )  
     &        +0.5*(1.-ZZ(K))*(ULPF(LEAST(L),K)*(BELV(LEAST(L))-BELV(L))*DXIU(LEAST(L))  
     &          +ULPF(L,K)*(BELV(L)-BELV(LWEST(L)))*DXIU(L)  
...
C  
        ...
        IF(IS3DUUU.GE.1)THEN  
          DO K=1,KC  
            TMP3D(K)=0.5*(ULPF(L,K)+ULPF(LEAST(L),K))  
          ENDDO  

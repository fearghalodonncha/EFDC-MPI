      ...
      IF(ISTL_.EQ.2)THEN  
        DO K=1,KS  
          DO L=2,LA  
            ...
            FUHU(L,K)=MAX(UHUW,0.)*QQ1(LWEST(L),K)  
     &          +MIN(UHUW,0.)*QQ1(L,K)  
            FVHU(L,K)=MAX(VHVW,0.)*QQ1(LS,K)  
     &          +MIN(VHVW,0.)*QQ1(L,K)  
            FUHV(L,K)=MAX(UHUW,0.)*QQL1(LWEST(L),K)*H1P(LWEST(L))  
        ...
        IF(ISCDCA(0).EQ.1)THEN  
          DO K=1,KS  
            DO L=2,LA  
              ...
              FUHU(L,K)=UHUW*(QQ(LWEST(L),K)+QQ(L,K))  
              FVHU(L,K)=VHVW*(QQ(LS,K)+QQ(L,K))  
              FUHV(L,K)=UHUW*(QQL(LWEST(L),K)*H1P(LWEST(L))+QQL(L,K)*H1P(L))  
        ...
        ELSE  
          DO K=1,KS  
            DO L=2,LA  
              ...
              UHUW=0.25*(UHDY2(L,K)+UHDY2(L,K+1))  
              VHVW=0.25*(VHDX2(L,K)+VHDX2(L,K+1))  
              FUHU(L,K)=MAX(UHUW,0.)*QQ2(LWEST(L),K)  
     &            +MIN(UHUW,0.)*QQ2(L,K)  
              FVHU(L,K)=MAX(VHVW,0.)*QQ2(LS,K)  
     &            +MIN(VHVW,0.)*QQ2(L,K)  
              FUHV(L,K)=MAX(UHUW,0.)*QQL2(LWEST(L),K)*H2P(LWEST(L))  
...
C  
     ...
     &            +DELT*(FUHU(L,K)-FUHU(LEAST(L),K)+FVHU(L,K)-FVHU(LN,K)  
     &            +(FWQQ(L,K)-FWQQ(L,K+1))*DZIG(K))*DXYIP(L)  
              VVV(L,K)=QQL1(L,K)*H1P(L)*H1P(L)  
     &            +DELT*(FUHV(L,K)-FUHV(LEAST(L),K)+FVHV(L,K)-FVHV(LN,K)  
     ...
     &            (U(LEAST(L),K+1)-U(LEAST(L),K)+U(L,K+1)-U(L,K))**2  
...
C  
      ...
      DO K=1,KS  
        DO LL=1,NCBW  
          L=LCBW(LL)  
          QQ(L,K)=QQ(LEAST(L),K)  
          QQL(L,K)=QQL(LEAST(L),K)  
          DML(L,K)=DML(LEAST(L),K)  
        ENDDO  
      ENDDO  
      DO K=1,KS  
        DO LL=1,NCBE  
          L=LCBE(LL)  
          QQ(L,K)=QQ(LWEST(L),K)  
          QQL(L,K)=QQL(LWEST(L),K)  
          DML(L,K)=DML(LWEST(L),K)  
        ENDDO  
      ENDDO  
      DO K=1,KS  
        DO LL=1,NCBN  

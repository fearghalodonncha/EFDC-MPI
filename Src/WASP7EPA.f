...
C
        DO L=1,LL,4
          WRITE(92,1024) QTMP(L), LUTMP(L), LDTMP(L), QTMP(LEAST(L)),LUTMP
     +    (LEAST(L)),LDTMP(LEAST(L)), QTMP(L+2),LUTMP(L+2),LDTMP(L+2), QTMP(L+3),
     +    LUTMP(L+3),LDTMP(L+3)
        END DO
C
        NBRKQ=6
     ...
     +      FORM='UNFORMATTED')
        ...
        DO K=KC,1,-1
          DO LT=2,LALT
            ...
            DEPTH=HLPF(L)*DZC(K)
            VELX=0.5*(UHLPF(L,K)+SVPT*UVPT(L,K) +UHLPF(LEAST(L),K)+SVPT*UVPT
     +      (LEAST(L),K))/HLPF(L)

...
C
      ...
      PARAMETER (LLCM=200)
      REAL CCW1(LLCM),CCE1(LLCM),CCN1(LLCM),CCS1(LLCM)
      REAL CCC1(LLCM)
...
C  
      IF(BSC.GT.1.E-6)THEN  
        DO L=2,LA  
          PMCTESTX(1,L)=-SBX(L)*HU(L)*((BI2(L)+BI2(LWEST(L)))*(HP(L)-HP(LWEST(L)))  
     &        +HU(L)*(BI1(L)-BI1(LWEST(L)))  
...
C  
     ...
     &      -DELTD2*SUB(L)*HRUO(L)*HUTMP(L)*(P(L)-P(LWEST(L)))  
...
C  
     ...
     &      ( SUB(LEAST(L))*PMCTESTX(2,LEAST(L))-SUB(L)*PMCTESTX(2,L)  
...
C  
      ...
      ENDDO  
      DO L=2,LA  
        !UHDYE(L)=SUB(L)*(FUHDYE(L)-DELTD2*HRUO(L)*RCX(L)*HUTMP(L)*(P(L)-P(LWEST(L))) )  
        PMCTESTX(4,L)=SUB(L)*( PMCTESTX(2,L) - 
     &   DELTD2*HRUO(L)*RCX(L)*HUTMP(L)*(PMCTESTY(3,L)-PMCTESTY(3,LWEST(L))))
...
C  
     ...
     &      -0.5*(PMCTESTX(4,LEAST(L))+PMCTESTX(6,LEAST(L))

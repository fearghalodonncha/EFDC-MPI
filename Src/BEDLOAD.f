        ...
        DO L=2,LA  
          ...
          IF(LMASKDRY(L))THEN  
            ...
            IF(UCELLCTR(L).GE.0.0.AND.VCELLCTR(L).GE.0.0)THEN  
              QSBDLDX(LEAST(L),NX)=SUB(LEAST(L))*QSBDLDP(L)*UCELLCTR(L)  
      ...
      IF(ISBLFUC.EQ.1)THEN  
        DO L=2,LA  
          ...
          IF(LMASKDRY(L))THEN  
            LN=LNC(L)  
            IF(UCELLCTR(L).GE.0.0.AND.VCELLCTR(L).GE.0.0)THEN  
              UCELLCTRM=SUB(LEAST(L))*ABS(UCELLCTR(L))  
              ...
              IF(VCELLCTRM.LT.1.0E-9)THEN  
                QSBDLDX(LEAST(L),NX)=SUB(LEAST(L))*SIGN(QSBDLDP(L),UCELLCTR(L))  
              ENDIF  
            ENDIF  
            IF(UCELLCTR(L).GE.0.0.AND.VCELLCTR(L).LT.0.0)THEN  
              UCELLCTRM=SUB(LEAST(L))*ABS(UCELLCTR(L))  
...
C  
     ...
     &        +QSBDLDP(LWEST(L))*UCELLCTR(LEAST(L)))  
        ...
        DO L=2,LA  
          IF(LMASKDRY(L))THEN  
            IF(QSBDLDX(L,NX).GT.0.0)THEN  
              SLOPE=(BELV(L)-BELV(LWEST(L)))*DXIU(L)  
              IF(SLOPE.GT.BLBSNT) QSBDLDX(L,NX)=0.0  
            ENDIF  
            IF(QSBDLDX(L,NX).LT.0.0)THEN  
              SLOPE=(BELV(LWEST(L))-BELV(L))*DXIU(L)  
              IF(SLOPE.GT.BLBSNT) QSBDLDX(L,NX)=0.0  
            ENDIF  
...
C  
      DO L=2,LA  
        IF(LMASKDRY(L))THEN  
          LN=LNC(L)  
          IF(UCELLCTR(L).GE.0.0.AND.VCELLCTR(L).GE.0.0)THEN  
            SNDFBL(L,NX)=DXYIP(L)*(QSBDLDX(LEAST(L),NX)+QSBDLDY(LN,NX)  
     &          +QSBDLDOT(L,NX))  
            ...
            IF(SNDBTMP.LT.0.0.AND.ASNDFBL.GT.0.0)THEN  
              ...
              QSBDLDX(LEAST(L),NX)=FLUXFAC*QSBDLDX(LEAST(L),NX)  
              QSBDLDY(LN ,NX)=FLUXFAC*QSBDLDY(LN ,NX)  
              QSBDLDOT(L ,NX)=FLUXFAC*QSBDLDOT(L ,NX)  
            ENDIF  
          ENDIF  
          IF(UCELLCTR(L).GE.0.0.AND.VCELLCTR(L).LT.0.0)THEN  
            SNDFBL(L,NX)=DXYIP(L)*(-QSBDLDY(L,NX)+QSBDLDX(LEAST(L),NX)  
     &          +QSBDLDOT(L,NX))  
            ASNDFBL=ABS(SNDFBL(L,NX))  
            SNDBTMP=SNDB(L,KBT(L),NX)-DELT*SNDFBL(L,NX)  
            IF(SNDBTMP.LT.0.0.AND.ASNDFBL.GT.0.0)THEN  
...
C  
      DO L=2,LA  
        ...
        CQBEDLOADY(L,NX)=0.0  
        IF(LMASKDRY(L))THEN  
          UVARTMP=0.5*(RSSBCW(L)*U(L,1)+RSSBCE(L)*U(LEAST(L),1))  
...
C  
      ...
      DO L=2,LA  
        IF(LMASKDRY(L))THEN  
          LN=LNC(L)  
          SNDFBL(L,NX)=DXYIP(L)*(QSBDLDX(LEAST(L),NX)  

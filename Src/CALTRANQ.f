...
C  
  300 CONTINUE  
      DO K=1,KS  
        DO L=2,LA  
          LS=LSC(L)  
          FUHU(L,K)=MAX(UHDY2(L,K),0.)*QCON1(LWEST(L),K)  
...
C  
  350 CONTINUE  
      ...
      DO K=1,KS  
        DO L=2,LA  
          LS=LSC(L)  
          FUHU(L,K)=MAX(UHDY2(L,K),0.)*CONT(LWEST(L),K)  
...
C  
  400 CONTINUE  
      DO K=1,KS  
        DO L=2,LA  
          LS=LSC(L)  
          FUHU(L,K)=0.5*UHDY2(L,K)*(QCON(L,K)+QCON(LWEST(L),K))  
      ...
      DO K=1,KS  
        ...
        DO LL=1,NCBW  
          L=LCBW(LL)  
          IF(UHDY2(LEAST(L),K).LT.0.) FUHU(LEAST(L),K)=UHDY2(LEAST(L),K)*QCON1(LEAST(L),K)  
        ENDDO  
        DO LL=1,NCBE  
          L=LCBE(LL)  
          IF(UHDY2(L,K).GT.0.) FUHU(L,K)=UHDY2(L,K)*QCON1(LWEST(L),K)  
        ENDDO  
        DO LL=1,NCBN  
          L=LCBN(LL)  
          LS =LSC(L)  
...
C  
      IF(NBCSOP.GT.0)THEN
        ...
        DO K=1,KS  
          DO LL=1,NCBW  
            L=LCBW(LL)  
            QCON(L,K)=QCON(LEAST(L),K)  
          ENDDO  
        ENDDO  
        DO K=1,KS  
          DO LL=1,NCBE  
            L=LCBE(LL)  
            QCON(L,K)=QCON(LWEST(L),K)  
          ENDDO  
        ENDDO  
        DO K=1,KS  
          DO LL=1,NCBN  

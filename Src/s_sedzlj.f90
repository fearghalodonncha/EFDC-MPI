SUBROUTINE SEDZLJ(L)
  ...
  REAL(4) SECNDS
  IF(IS_TIMING)T1TMP=SECNDS(SECNDS_ZERO)  
  !
  ...
  IF (KC == 1) THEN
     DO K=1,NSCM
        VZDIF(L)=MAX(20.d0,0.067d0*HT(L)*USTAR(L)*100.d0) !PT due to units of USTAR calc. in s_shear.f90
  ...
  DO K=1,NSCM !loop through all sediment sizes
     IF(CTB(K,L)<1.0E-20)CYCLE !if there is no sediment of that size available in the water column, then it cannot be deposited
...
! Size class.  If there is one, we need an active layer.
  ...
  ALL_LAYERS:DO LL=1,KB !go through all sediment layers so that they are properly eroded and sorted
     ...
     ELSE
        ...
        IF(LL+1<=KB)THEN !Avoid array exceedance when LL=KB and used in TAUCOR(L+1,L)
     ...
     ELSE
        ...
        ERATEMOD(L)=(SN00*EXP(SN11*LOG(ERATEND(NSC0,NTAU0))+SN01* &
        LOG(ERATEND(NSC1,NTAU0)))+SN10*EXP(SN11*LOG(ERATEND(NSC0,NTAU1))+ &
        SN01*LOG(ERATEND(NSC1,NTAU1))))*BULKDENS(LL,L)*SQRT(1./SH_SCALE(L)) !log-linear interpolation
     ENDIF

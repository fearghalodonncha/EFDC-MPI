      SUBROUTINE CALTOXB  
C
C **  SUBROUTINE CALTOXB CALCULATES CONTAMINANT TRANSPORT 
C **  WITHIN THE SEDIMENT BED, INCLUDING FLUX OF CONTAMINANTS
c **  FROM THE TOP BED LAYER TO THE WATER COLUMN.
C
      USE GLOBAL
...
C  
     ...
     &            (1.-TOXPFTB(L,K,NT))  
            ...
            ENDDO  
            RRHS(L,1)=RRHS(L,1)+MAX(QWTRBED(L,0),0.0)*CONGW(L,NT+4)  

...
C  
      ...
      DO L=2,LA  
        ...
        WVWHAUT=(WVWHA(L)+SUB(L)*WVWHA(LWEST(L)))/(1.+SUB(L))  
...
C  
     ...
     &          *( RSWRSI*(DYU(L)*(WVPP(L,K)-WVPP(LWEST(L),K))  
     &          +DYU(L)*WVPU(L,K)*(HMPW(L)-HMPW(LWEST(L))))  
     &          +RSWRSR*(DYP(L)*WVHUU(L,K)-DYP(LWEST(L))*WVHUU(LWEST(L),K)  
     &          +0.5*(DXV(LN )+DXV(LNW))*WVHUV(LN,K)      !
     &          -0.5*(DXV(L  )+DXV(LWEST(L)))*WVHUV(L ,K)) )  
     ...
     &          +0.5*(DYU(LEAST(L))+DYU(LSE))*WVHUV(LEAST(L),K)     !
